...
from modules.agent_codegen_runtime import analyze_codegen_prompt, build_codegen_filename, build_run_command
from modules.codegen_utils import sanitize_llm_code, compile_check
...
def generate_and_run(prompt, context=None):
    """
    Główne wejście do silnika codegen.
    """
    try:
        # 1. Analiza promptu
        analysis = analyze_codegen_prompt(prompt)

        # 2. Generowanie kodu (tu masz już swoje LLM call)
        code = analysis.get("code", "")

        # 3. Sanitizacja
        safe_code = sanitize_llm_code(code)
        ...
        if "hello world" in prompt.lower():
            ...
            filename = build_codegen_filename(prompt)
        ...
        if not filename.endswith(".py"):
            ...
            # 5. Kompilacja / sprawdzenie
            compile_check(filename)
