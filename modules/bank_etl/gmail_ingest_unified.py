...
# ====== SQL: tabela docelowa + migracje (BEZ UNIQUE w ALTER TABLE) ======
def ensure_db():
    ...
    # Dołóż brakujące kolumny — bez UNIQUE w definicji
    ...
    # 2) Jednorazowe odduplikowanie istniejących rekordów (na wszelki wypadek)
...
def detect_block(text: str) -> str:
    ...
    if ("przelew na telefon" in low) or ("blik na telefon" in low) or ("blik p2p" in low):
        return "phone"
    # UZNANIE/ZWROT – łap szeroko, bez wymogu układu linii
...
def parse_message(msg: dict):
    ...
    if block == "card_auth":
        ...
        # Analizujemy tylko część PRZED „Dostępne” i/lub „Data waluty”
        low = text.lower()
        cut_pos = []
        ...
        # Zbierz wszystkie dodatnie kwoty +... PLN z sekcji (bez salda)
        plus_re = re.compile(r"\+\s*([\d\s\u00A0]+(?:[.,]\d{2})?)\s*(?:PLN|zł)\b", re.I)
        plus_amounts = [norm_amt(m.group(1)) for m in plus_re.finditer(section)]

        if plus_amounts:
            # Największa dodatnia kwota to najbardziej prawdopodobne właściwe UZNANIE
            amount = max(plus_amounts)
            direction = "in"
            source_hint = "mail:income_zwrot" if "zwrot" in low else "mail:income"
        else:
            # Awaryjnie: jakakolwiek kwota z sekcji (jeśli nie znaleziono +…)
        ...
        if amount is not None:
            ...
            if not counterparty:
                # Fallback – linia po „UZNANIE”/„ZWROT”
                lns = to_lines(section)
                idx = next((i for i, ln in enumerate(lns) if ("uznanie" in ln.lower() or "zwrot" in ln.lower())), None)
        ...
        # Płatność/przelew BLIK na telefon – również tniemy sekcję przed „Data waluty”
        section = text.split("Data waluty")[0]
        m_plus = RE_PLUS_AMT.search(section)
        m_minus = RE_MINUS_AMT.search(section)
        m_any = RE_ANY_AMT.search(section)
        if m_plus:
            amount = abs(norm_amt(m_plus.group(1))); direction = "in"
        elif m_minus:
            amount = -abs(norm_amt(m_minus.group(1))); direction = "out"
        elif m_any:
            a = norm_amt(m_any.group(1))
            hint = text.lower()
            if any(w in hint for w in ["otrzymałe", "przychodzą", "uznanie"]):
                amount = abs(a); direction = "in"
            else:
                amount = -abs(a); direction = "out"
        if amount is not None:
            source_hint = "mail:phone"
            m_cp = RE_NADAWCA.search(text)
            m_od = RE_ODBIORCA.search(text)
            counterparty = (
                normalize_counterparty(m_cp.group(1)) if m_cp
...
def _backfill_missing_hashes(con: sqlite3.Connection):
    ...
    cur = con.cursor()
    rows = cur.execute("""
        SELECT rowid AS rid, id, ym, op_date, value_date, direction, amount, currency, title,
               counterparty, source_hint, tx_hash, biz_hash, created_at
    ...
    for r in rows:
        ...
        try:
            cur.execute("UPDATE transactions_final SET tx_hash=?, biz_hash=? WHERE rowid=?", (txh, bizh, r["rid"]))
        except sqlite3.IntegrityError:
            # Już istnieje nowszy rekord z tym samym biz_hash → usuń starszy bieżący
            cur.execute("DELETE FROM transactions_final WHERE rowid=?", (r["rid"],))
    con.commit()

def _dedupe_existing(con: sqlite3.Connection):
    """
    Usuwa duplikaty wg biz_hash zostawiając najnowszy (MAX(rowid)).
    Zero CTE i funkcji okienkowych – działa na każdym SQLite.
    """
    cur = con.cursor()
    cur.execute("""
...
def upsert_row(cur, row: dict):
    ...
    try:
        ...
        cur.execute(
            """SELECT is_pending, sources_seen, first_source
                 FROM transactions_final
                WHERE biz_hash=?""",
            (biz_hash,)
        )
        ...
        new_pending = 0 if row["source_hint"] in ("mail:charge", "mail:income", "mail:income_zwrot") else was_pending
        cur.execute(
...
def build_query(user_q: str) -> str:
    uq = (user_q or "").strip()
    # jeśli użytkownik podał własne warunki (np. "from:", "subject:", "label:"), przepuść bez zmian
    if "from:" in uq or "subject:" in uq or "label:" in uq:
        return uq
    # domyślnie zawężamy do Inteligo, ale pozwalamy nadpisać zapytanie parametrem
