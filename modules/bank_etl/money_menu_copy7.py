...
import os, sqlite3, datetime, sys, re, subprocess, time
...
def ensure_transactions_up_to_date():
    ...
    • jeśli token wygasł → money nie startuje, kręci się w pętli, aż odświeżysz token
    • jeśli Gmail rzuci inny błąd → zatrzymuje money, żeby nie pracować na pustej bazie
    """
    ingest_script = os.path.expanduser("~/HALbridge/modules/bank_etl/gmail_ingest_unified.py")
    ingest_cmd = [sys.executable, ingest_script, "newer_than:120d"]
    ...
    while True:
        try:
            ...
            # sukces ingestu
            if "@#@Ingest OK" in out:
                print("[OK] Ingest zakończony. Dane są aktualne.\n")
                return

            # token expired / invalid_grant / refresh
            low = out.lower()
            if ("invalid_grant" in low) or ("token" in low and "expired" in low) or ("refresh" in low and "token" in low):
                print("\n[ŻÓŁTA LAMPKA] Token Gmail wygasł.")
                print("Odśwież token w przeglądarce, Jaśnie Panie.")
                print("Ponawiam próbę za 10 sekund...\n")
                time.sleep(10)
                continue

            # inne błędy Gmail
            print("\n[ERROR] Nie udało się pobrać transakcji z Gmaila:")
            print(out)
            print("Zatrzymuję money, aby baza nie była pusta.")
            sys.exit(1)

        except Exception as e:
            print(f"[CRITICAL] Błąd systemowy ingestu: {e}")
            time.sleep(5)
            continue
