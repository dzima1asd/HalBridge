...
def list_incomes(con: sqlite3.Connection, ym: str):
    # wykluczamy przelewy na telefon
    return q(con, """
...
def show_rows(title: str, rows, is_out=False) -> None:
    ...
    t = Table(
        ...
        style="green",
        expand=True,          # wypełnij całą szerokość terminala
        show_lines=False,
    )

    # pełna data (YYYY-MM-DD), bez zawijania
    t.add_column("Data", style="cyan", width=10, no_wrap=True)
    # kwota węższa, wyrównana do prawej
    t.add_column("Kwota", style="bright_yellow", justify="right", no_wrap=True)
    # dłuższe pola mogą się zawijać, nie ucinać
