...
# =========================
def load_service():
    """Stabilna wersja Gmail OAuth – refresh token działa bez końca."""

    os.makedirs(CONF, exist_ok=True)
    creds = None

    # 1. Wczytaj istniejący token jeśli jest
    ...
    # 2. Jeśli token jest, ale nieważny – spróbuj odświeżyć
    if creds and creds.expired and creds.refresh_token:
        try:
            ...
            # jeśli refresh się wywali – wymusimy nowy login
            creds = None

    # 3. Jeśli brak tokena lub refresh się nie udał → nowy OAuth (RAZ NA LATA)
    if creds is None or not creds.valid:
        print("🔑 Brak ważnego tokena Gmail – uruchamiam jednorazowy flow OAuth.")
        ...
        # UWAGA: run_console() NIE DODAJE prompt=consent
        # więc refresh_token będzie działał stabilnie
        creds = flow.run_console()

        # ZAPISZ refresh_token na później
        with open(TOKEN_JSON, "w") as f:
            f.write(creds.to_json())

        print("✅ Token Gmail zapisany (jednorazowa autoryzacja).")
