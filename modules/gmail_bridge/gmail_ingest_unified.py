...
FUEL_VENDORS = ["orlen","pkn orlen","circle k","bp","moya","lotos","avia","amic","shell","statoil","total","stacja paliw","port24"]
...
def detect_block(text: str) -> str:
    ...
    if ("przelew na telefon" in low) or ("blik na telefon" in low) or ("blik p2p" in low):
        return "phone"
    if "\nuznanie\n" in low or " uznanie" in low or "przelew przych" in low or "przychodz" in low or "zwrot" in low:
        return "income"
    return "unknown"
...
def parse_message(msg: dict):
    ...
    if block == "card_auth":
        ...
        if m_plus:
            ...
            hint = text.lower()
            if any(w in hint for w in ["otrzymałe", "przychodzą", "uznanie", "otrzymano"]):
        ...
        if amount is not None:
            ...
            counterparty = (normalize_counterparty(m_cp.group(1)) if m_cp else
                            (normalize_counterparty(m_od.group(1)) if m_od else counterparty))
...
# ====== Backfill + dedupe ======
def _backfill_missing_hashes(con):
    cur = con.cursor()
    rows = cur.execute("SELECT rowid, op_date, amount, counterparty, currency, direction FROM transactions_final WHERE tx_hash IS NULL OR biz_hash IS NULL").fetchall()
    for rid, op_date, amount, counterparty, currency, direction in rows:
        row = {"op_date": op_date, "amount": amount, "counterparty": counterparty, "currency": currency, "direction": direction}
        txh = tx_hash_for(row)
        bzh = biz_hash_for(row)
        try:
            cur.execute("UPDATE transactions_final SET tx_hash=?, biz_hash=?, updated_at=? WHERE rowid=?", (txh, bzh, now_iso(), rid))
        except sqlite3.IntegrityError:
            cur.execute("DELETE FROM transactions_final WHERE rowid=?", (rid,))
    con.commit()

def _dedupe_existing(con):
    cur = con.cursor()
    cur.execute("DELETE FROM transactions_final WHERE rowid NOT IN (SELECT MIN(rowid) FROM transactions_final GROUP BY tx_hash)")
    cur.execute("DELETE FROM transactions_final WHERE biz_hash IS NOT NULL AND rowid NOT IN (SELECT MIN(rowid) FROM transactions_final GROUP BY biz_hash)")
    con.commit()
...
def upsert_row(cur, row: dict):
    ...
    try:
        ...
        """, (
            ...
            row["title"], row["counterparty"], row["source_hint"],
            tx_hash, created_at, biz_hash, row["is_pending"], row["category"],
            row["source_hint"], row["source_hint"], updated_at
        ))
        return 1
    except sqlite3.IntegrityError:
        cur.execute("SELECT is_pending, sources_seen FROM transactions_final WHERE biz_hash=? OR tx_hash=? LIMIT 1", (biz_hash, tx_hash))
        r = cur.fetchone()
        if not r:
            return 0
        was_pending, sources_seen = r
        ...
        cur.execute("UPDATE transactions_final SET sources_seen=?, is_pending=?, updated_at=? WHERE biz_hash=? OR tx_hash=?", (",".join(sorted(sources)), new_pending, now_iso(), biz_hash, tx_hash))
        return 0
