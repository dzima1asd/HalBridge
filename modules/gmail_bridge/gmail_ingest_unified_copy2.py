...
def detect_block(text: str) -> str:
    ...
    if "przelew na telefon" in low or "blik na telefon" in low or "blik p2p" in low: return "phone"
    return "unknown"
...
def parse_message(msg: dict):
    ...
    if block == "card_auth":
        ...
        if amount is not None:
            ...
            m_cp = RE_NADAWCA.search(text)
            m_od = RE_ODBIORCA.search(text)
            counterparty = normalize_counterparty(m_cp.group(1)) if m_cp else (normalize_counterparty(m_od.group(1)) if m_od else counterparty)
...
def build_query(user_q: str) -> str:
    ...
    if "from:" in uq:
        return uq
    # domyślnie zawężamy do Inteligo
