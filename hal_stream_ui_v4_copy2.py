        ...
        if topic == T_STATUS:
            try:
                ...
                if isinstance(stats, dict):
                    state["segments_generated"] = int(stats.get("segments_generated", state["segments_generated"]) or 0)
                    state["stream_uptime"] = int(stats.get("duration", state["stream_uptime"]) or 0)
                    state["photos_taken"] = int(stats.get("photos_taken", state["photos_taken"]) or state["photos_taken"])
                    state["recordings_count"] = int(stats.get("recordings_count", state["recordings_count"]) or state["recordings_count"])
            ...
            try:
                ...
                if event_type == "photo_captured":
                    ...
                    state["last_event"] = f"❌ Error: {str(details.get('message','Unknown'))[:30]}"
                elif event_type == "warning":
                    state["last_event"] = f"⚠ Warning: {str(details.get('message','Unknown'))[:30]}"
                elif event_type == "stream_started":
                    state["stream"] = True
                elif event_type == "stream_stopped":
                    state["stream"] = False
            ...
            lines = [ln.rstrip() for ln in str(diag_text).splitlines() if ln.strip()]
            ...
            try:
                ...
                if cmd == "photo" and res == "OK":
                    ...
                    state["last_event"] = "🎬 Recording stopped"
                    state["recording_active"] = False
                elif cmd in ("stream_on", "stream_off", "start", "stop") and res == "OK":
                ...
                elif cmd in ("rec_motion_on", "mrec_on") and res == "OK":
                    state["last_event"] = "🧠 Motion → recording ON"
                elif cmd in ("rec_motion_off", "mrec_off") and res == "OK":
                    state["last_event"] = "🧠 Motion → recording OFF"
                elif cmd.startswith("profile") and res == "OK":
                    state["last_event"] = f"🎛 {msg_txt}" if msg_txt else "🎛 Profile set"
                elif cmd.startswith("tailscale") and res == "OK":
                    state["last_event"] = f"🌐 {msg_txt}" if msg_txt else "🌐 Network mode changed"
                elif msg_txt:
    ...
    while True:
        ...
        if k == ord('1'):
            with lock:
                ...
                if want_off:
                    state["stream"] = False      # optimistic update
                    _publish_cmd("stop")
                else:
                    state["stream"] = True       # optimistic update
                    _publish_cmd("start")

                state["last_event"] = "⏳ Switching stream..."
            run_diagnostics()
            ...
            # nowa komenda: mrec_on/off (dla motion->record)
            _publish_cmd("mrec_off" if want_off else "mrec_on")
            run_diagnostics()
