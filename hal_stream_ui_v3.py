...
state = {
    ...
    # Health components - zgodnie z UI
    "pi_ok": False,
    "ssh_ok": False,
    "camera_ok": False,
    "ffmpeg_ok": False,
...
def publish_status(event=None, err=None, force=False):
    ...
    # Aktualizuj health components na podstawie stanu
    with lock:
        # Ustaw health na podstawie aktualnego stanu
        if proc["pi"] and proc["pi"].poll() is None:
            state["pi_ok"] = True
        ...
        if proc["hls"] and proc["hls"].poll() is None:
            state["ffmpeg_ok"] = True
            state["hls_ok"] = hls_ready()
        else:
        ...
        state["mqtt_ok"] = client is not None and client.is_connected()
    
    # Przygotuj pełną strukturę zgodną z UI
    full_status = {
        "stream": state["stream"],
        ...
        "features": {
            ...
            "manual_record": state["manual_record"],
            "auto_record": state["auto_record"]
        },
        "stats": {
            "photos_taken": state["photos_taken"],
            "recordings_count": state["recordings_count"],
            "segments_generated": state["segments_generated"],
            "duration": time.time() - state["stream_started_at"] if state["stream_started_at"] > 0 else 0
        },
        "health": {
            "pi_camera": "healthy" if state["pi_ok"] else "failed",
            "ssh": "healthy" if state["ssh_ok"] else "failed",
            "camera": "healthy" if state["camera_ok"] else "failed",
            "ffmpeg": "healthy" if state["ffmpeg_ok"] else "failed",
            "http": "healthy" if state["http_ok"] else "failed",
            "hls": "healthy" if state["hls_ok"] else "failed",
            "mqtt": "healthy" if state["mqtt_ok"] else "failed"
        }
    }
    
    payload = json.dumps(full_status, ensure_ascii=False)
    now = time.time()
    ...
    # Publikuj również health osobno
    publish_health()
...
def publish_health():
    ...
    _publish(T_HEALTH, json.dumps({"health": health_data}, ensure_ascii=False))
...
def publish_event(event_type, details=None):
    ...
    _publish(T_EVENT, json.dumps(event, ensure_ascii=False))
...
def take_photo():
    ...
    try:
        ...
        # Aktualizuj statystyki i publikuj event
        with lock:
        ...
        # Publikuj event
        publish_event("photo_captured", {
            "counter": state["photos_taken"],
            "filename": filename
        })
...
def start_recording():
    ...
    try:
        ...
        if not state["stream"]:
            ack("rec_on", False, "Stream not running")
            return False
        ...
        # Publikuj event
        publish_event("recording_started", {
            "counter": state["recordings_count"],
            "filename": filename
        })
        
        log(f"Recording to: {filename}")
        ack("rec_on", True, f"Recording to {filename}")
        publish_status("recording_started")
        return True
...
def stop_recording():
    ...
    try:
        ...
        publish_status("recording_stopped")
        return True
...
def generate_diagnostics():
    ...
    try:
        ...
        # Dodaj health info
        diag_lines.append("")
        diag_lines.append("=== HEALTH ===")
        diag_lines.append(f"Pi Camera: {'healthy' if state['pi_ok'] else 'failed'}")
        diag_lines.append(f"SSH: {'healthy' if state['ssh_ok'] else 'failed'}")
        diag_lines.append(f"Camera: {'healthy' if state['camera_ok'] else 'failed'}")
        diag_lines.append(f"FFmpeg: {'healthy' if state['ffmpeg_ok'] else 'failed'}")
        diag_lines.append(f"HTTP: {'healthy' if state['http_ok'] else 'failed'}")
        diag_lines.append(f"HLS: {'healthy' if state['hls_ok'] else 'failed'}")
        diag_lines.append(f"MQTT: {'healthy' if state['mqtt_ok'] else 'failed'}")
...
def start_stream():
    ...
    publish_event("stream_started")
    ack("stream_on", True, STREAM_URL)

def stop_stream():
    cleanup("stream_off")
    publish_event("stream_stopped")
...
def on_message(c, _, msg):
    ...
    if cmd == "stream_on":
        ...
        threading.Thread(target=stop_stream, daemon=True).start()
    elif cmd == "status":
        publish_status("status_requested", force=True)
    elif cmd == "diag":
        threading.Thread(target=generate_diagnostics, daemon=True).start()
    elif cmd == "photo":
...
def main():
    ...
    # Publikuj początkowy status
    publish_status("engine_started", force=True)
